...
<template>
  <div class="mem-chart">
    <svg width="100%" class="mem-chart__svg" />
  </div>
</template>
...
export default {
  ...
  methods: {
    drawChart(_data) {
      ...
      //draw max
      const max = vm.areas.selectAll(".mem-chart__line--max").data([data]);
      max
        .enter()
        .append("path")
        .merge(max)
      ...
      const used = vm.areas.selectAll(".mem-chart__area--used").data([data]);
      used
        .enter()
        .append("path")
        .merge(used)
      ...
      vm.yAxis.call(d3.axisLeft(y).ticks(5).tickFormat(prettyBytes));
    },
  },
  mounted() {
    const margin = {
    ...
    this.chartLayer = d3
      .select(this.$el.querySelector(".mem-chart__svg"))
      .append("g")
