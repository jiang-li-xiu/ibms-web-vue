 ...
 * @LastEditTime: 2022-06-06 08:48:22
-->
<template>
  <div class="release-eqpt-table">
    ...
    <el-dialog :title="is_edit" :visible.sync="dialogVisible" width="40%">
      ...
      <el-form
        ...
        :rules="rules"
      >
        <el-form-item label="发布消息" prop="id">
          <el-select v-model="taskParameters.id" placeholder="请选择消息">
            <el-option
              v-for="item in msgList"
              :key="item.id"
              :label="item.programName"
              :value="item.programDesc"
            >
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="优先级 " prop="level">
          <el-input-number :max="100" :min="1" v-model="taskParameters.level">{{
            taskParameters.level
          }}</el-input-number>
        </el-form-item>
        <!-- 定时发布 -->
        <div v-show="taskParameters.pattern == '3'">
          ...
          >
            <el-input
              v-model="taskParameters.startCron"
            ...
            >
              <el-button
                ...
                @click="(cronVisible = true), (cronType = 0)"
              ></el-button>
            </el-input>
          </el-form-item>
        </div>
        ...
        <div v-show="taskParameters.pattern == '3'">
          ...
          >
            <el-input
              v-model="taskParameters.endCron"
              placeholder="请点击右侧按钮选择corn表达式"
              :disabled="true"
            >
              <el-button
                slot="append"
                icon="el-icon-alarm-clock"
                @click="(cronVisible = true), (cronType = 1)"
              ></el-button>
            </el-input>
          </el-form-item>
        </div>
        ...
        <el-form-item label="发布设备" prop="dialogTableData">
          <el-button
            ...
            :icon="
              taskParameters.dialogTableData.length == 0
                ? 'el-icon-plus'
                : 'el-icon-edit'
            "
            >{{
    ...
    >
      <cron
        v-model="taskParameters.startCron"
        v-if="cronVisible && cronType == 0"
      />
      <cron
        v-model="taskParameters.endCron"
        v-else-if="cronVisible && cronType == 1"
      />
    ...
    >
      ...
      <el-row class="query" :gutter="20">
        ...
        <el-col :span="4">
          <el-button @click="getMsgs('refresh')" icon="el-icon-refresh"
            >重置</el-button
          >
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      taskParameters: {
        ...
        programCode: null, //发布消息编号
        programDesc: null, //发布消息编号
        pattern: "1", //发布模式
        status: "发布", //发布状态
        startCron: "", //Cron表达式
        endCron: "", //Cron表达式
  ...
  methods: {
    ...
    //提交
    submitClickConnect() {
      this.$refs["taskParameters"].validate((valid) => {
        console.log(valid);
        if (valid) {
          let dataObject = {
            id: this.taskParameters.id, //发布消息id
            pattern: this.taskParameters.pattern, //发布模式
            level: this.taskParameters.level, //优先级
            status: this.taskParameters.status, //发布状态
          ...
          if (dataObject.pattern == "3") {
            dataObject.startCron = this.taskParameters.startCron;
            dataObject.endCron = this.taskParameters.endCron;
          }
          let funAddAandEdit;
    ...
    // 打开编辑
    async edit(row) {
      let { rows } = await getProgramList();
      console.log(rows);
      this.msgList = await rows.map((item) => {
        console.log(item);
        return item;
      });
      ...
      if (row) {
        ...
        this.taskParameters = {
          id: row.id, //id
          programId: row.programId, //发布消息id
          pattern: row.pattern, //发布模式
          status: row.status, //发布状态
          startCron: row.startCron, //Cron表达式
          endCron: row.endCron, //Cron表达式
          level: row.level, //优先级
        };
