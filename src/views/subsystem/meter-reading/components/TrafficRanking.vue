 ...
 * @Descripttion: 
 * @Author: ZhouFanSong
 * @Date: 2021-09-27 11:50:44
 * @LastEditors: ZhouFanSong
 * @LastEditTime: 2021-12-02 16:11:59
-->
<template>
  <div>
...
import CuboidBardiagram from "@/components/Echarts/HorizontalColumn.vue";
export default {
  props: {
    title: {
      type: String,
  ...
  methods: {
    ...
    getList(type) {
      ...
      getBuildingUse(this.queryParams).then((response) => {
        ...
        this.dataList.legend = obj.meterName;
        let seriesData = [];
        for (let key in obj.meterName) {
          seriesData.push({ name: obj.meterName[key], value: obj.values[key] });
        }
        this.dataList.seriesData = seriesData;
