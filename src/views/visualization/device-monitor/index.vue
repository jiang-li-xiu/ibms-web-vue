 ...
 * @LastEditTime : 2022-04-19 17:48:00
 * @LastEditors  : Imor
 * @FilePath     : \ibms-web-vue\src\views\visualization\device-monitor\index.vue
 * @Description  : 
-->
<template>
  <div class="app-container canvas_manager">
    ...
    <div class="district_divide flex">
      ...
      <div class="middle_view">
        ...
        <div class="shortcut_help">
          ...
          >
            <div slot class="edit_helper_text">
              <b>1.显示监控视频：</b>鼠标双击图标可展示视频监控；<br />
              <b>2.关闭监控视频：</b>鼠标双击视频或点击关闭按钮；<br />
              <!-- <b>3.监控视频拖拽：</b>鼠标单击可拖拽移动视频弹窗；<br /> -->
              <b>3.显示设备信息：</b
              ...
              <b>6.区域查询：</b>区域列表的搜索栏可通过模糊搜索查询区域；
              <p style="color: red">
        ...
        <xg-player-single
          class="video_player"
          :source.sync="monitorURL"
          :items="playerItem"
          :streamType="streamType"
          :playerShow="playerShow"
          @close_widow="changePlayerShow"
          ref="refXgPlayer"
        />
      </div>
    </div>
...
//视频监控
import XgPlayerSingle from "@/components/XgPlayer/XpPlayerSingle.vue";
import { getUrlByChannelId } from "@/api/subsystem/video-monitoring/videomonitor";
//局部共用组件
...
export default {
  ...
  methods: {
          ...
          .then((res) => {
            ...
            this.modelList.forEach((child) => {
              if (child.set) {
                ...
                if (parseInt(childType.split("m")[1]) < 3) {
    ...
    handleNodeClick(data) {
      this.$refs.refXgPlayer.originWindow();
  ...
  watch: {
    ...
    deviceId(newVal) {
      if (newVal != "") {
        // this.monitorURL =
        //   "ws://192.168.1.240:20101/live?token=e829e0be0ca16b95dfad8d3085aa849005c0994dab928832642dd19c5e2f95d30dfac7c411444736e39228363da7f97184046a4b76ecbca91161a6a87498a5a1";
        // this.playerShow = true;
        // this.playerItem = {
        //   channelId: "61e7ca59fb42c075056f1c78",
        //   uri: "ws://192.168.1.240:20101/live?token=e829e0be0ca16b95dfad8d3085aa849005c0994dab928832642dd19c5e2f95d30dfac7c411444736e39228363da7f97184046a4b76ecbca91161a6a87498a5a1",
        //   index: 0,
        // };
        // this.$refs.refXgPlayer.initPlay(this.monitorURL);
        getUrlByChannelId({ deviceCode: newVal })
          .then((res) => {
            console.log(res);
            this.monitorURL = res.data.uri;
            this.playerShow = true;
            this.playerItem = {
              channelId: newVal,
              uri: res.data.uri,
