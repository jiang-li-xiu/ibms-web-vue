...
let s_progress = function () {
  let player = this
  let container = createDom('xg-shaft', 
  `<xg-outer class="xgplayer-shaft-outer">
        <xg-cache class="xgplayer-shaft-cache"></xg-cache>
        <xg-played class="xgplayer-shaft-played">
            <xg-shaft-btn class="xgplayer-shaft-btn"></xg-shaft-btn>
            <xg-point class="xgplayer-shaft-point xgplayer-tips"></xg-point>
            <xg-thumbnail class="xgplayer-shaft-thumbnail xgplayer-tips"></xg-thumbnail>
        </xg-played>
    </xg-outer>`
, {tabindex: 1}, 'xgplayer-shaft')

  let containerWidth
  player.controls.appendChild(container)
  // 进度条
  let progress = container.querySelector('.xgplayer-shaft-played')
  let outer = container.querySelector('.xgplayer-shaft-outer')
//   缓存
  let cache = container.querySelector('.xgplayer-shaft-cache')
//   前端点
  let point = container.querySelector('.xgplayer-shaft-point')
  let thumbnail = container.querySelector('.xgplayer-shaft-thumbnail')
  player.dotArr = {}
  function dotEvent (dotItem, text) {
    dotItem.addEventListener('mouseenter', function (e) {
      if (text) {
        addClass(dotItem, 'xgplayer-shaft-dot-show')
        addClass(container, 'xgplayer-shaft-dot-active')
      }
    })
    dotItem.addEventListener('mouseleave', function (e) {
      if (text) {
        removeClass(dotItem, 'xgplayer-shaft-dot-show')
        removeClass(container, 'xgplayer-shaft-dot-active')
      }
    })
    dotItem.addEventListener('touchend', function (e) {
      ...
      e.stopPropagation()
      if (text) {
        if (!hasClass(dotItem, 'xgplayer-shaft-dot-show')) {
          Object.keys(player.dotArr).forEach(function (key) {
            if (player.dotArr[key]) {
              removeClass(player.dotArr[key], 'xgplayer-shaft-dot-show')
            }
          })
        }
        toggleClass(dotItem, 'xgplayer-shaft-dot-show')
        toggleClass(container, 'xgplayer-shaft-dot-active')
      }
    })
  }
  function onCanplay () {
    if (player.config.progressDot && typeOf(player.config.progressDot) === 'Array') {
      player.config.progressDot.forEach(item => {
        if (item.time >= 0 && item.time <= player.duration) {
          let dot = createDom('xg-shaft-dot', item.text ? `<span class="xgplayer-shaft-tip">${item.text}</span>` : '', {}, 'xgplayer-shaft-dot')
  ...
  player.addProgressDot = function (time, text, duration, style) {
    ...
    if (time >= 0 && time <= player.duration) {
      let dot = createDom('xg-shaft-dot', text ? `<span class="xgplayer-shaft-tip">${text}</span>` : '', {}, 'xgplayer-shaft-dot')
  ...
  container.addEventListener('mouseenter', function (e) {
    ...
    let compute = function (e) {
      ...
      if (hasClass(container, 'xgplayer-shaft-dot-active')) {
